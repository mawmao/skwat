...
fun FormWizardScreen() {
    ...
    Column(
        ...
        AnimatedVisibility(
            ...
            enter = slideInVertically(animationSpec = defaultTween(150)) { it },
            exit = slideOutVertically(animationSpec = defaultTween(150)) { it }
        ) {
            Column(
                ...
                ScoutButton(
                    ...
                    onClick = if (formState.canScrollNext) formState::scrollWizardNext else formNavigator::navigateToFormReview
                )
            }
        }
    }
...
fun WizardPager(modifier: Modifier = Modifier) {
    ...
    HorizontalPager(
        ...
        val renderer = pageOverrides[pageKey] ?: { DefaultWizardEntry(it) }
        renderer(pageKey)
    }
}
